...
export class IndexedDBDocStorageAdapter extends DocStorageAdapter {
  private db!: IDBPDatabase<DocStorageSchema>;
  constructor(
  ...
  override async connect(): Promise<void> {
    const tryConnect = async () => {
      this.db = await openDB<DocStorageSchema>(
        `workspace:${this.spaceId}`,
        latestVersion,
        {
          upgrade: migrate,
          blocking: () => {
